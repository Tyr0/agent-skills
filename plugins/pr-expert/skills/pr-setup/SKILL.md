...
```bash
TOKEN=$(security find-generic-password -w -s 'tcalderone-agent-skills:github' -a 'default')
```
...
")
BB_API_TOKEN=$(security find-generic-password -w -s 'tcalderone-agent-skills:bitbucket' -a "$BB_EMAIL")
```
...
3. Register the token (paste at the prompt):
   ...
   ```bash
   security add-generic-password -U -s 'tcalderone-agent-skills:github' -a 'default'
   ```
...
3. Register the token under your Atlassian email (replace `<email>`):
   ...
   ```bash
   security add-generic-password -U -s 'tcalderone-agent-skills:bitbucket' -a '<email>'
   ```
