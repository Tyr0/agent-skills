...
| Operation | Command |
|---|---|
| Set or rotate (macOS) | `security add-generic-password -U -s 'open-agent-skills:<name>' -a '<account>' -w "$value"` |
| Read (macOS) | `security find-generic-password -w -s 'open-agent-skills:<name>' -a '<account>'` |
| Delete (macOS) | `security delete-generic-password -s 'open-agent-skills:<name>' -a '<account>'` |
...
|---|---|
| `macos-keychain` (default on Darwin) | `service = "open-agent-skills:<name>"`, `account = entry.account or "default"` |
| `secret-service` (default on Linux) | attrs `{application: "open-agent-skills", credential: "<name>", account: entry.account or "default"}` |
| `windows-credential-manager` (default on Windows) | `target = "open-agent-skills:<name>"`, `username = entry.account or "default"` |
...
service="open-agent-skills:$name"
...
```bash
GITHUB_TOKEN=$(security find-generic-password -w -s 'open-agent-skills:github' -a 'default') \
  gh pr list
unset GITHUB_TOKEN
```
...
security delete-generic-password -s "open-agent-skills:$name" -a "$account"
...
")
  ...
  case "$backend" in
    macos-keychain)
      security find-generic-password -w -s "open-agent-skills:$name" -a "$account"
      ;;
    secret-service)
      secret-tool lookup application open-agent-skills credential "$name" account "$account"
      ;;
    *)
...
| Editing the index to use a custom keystore service name | Diverges from the keying convention; entry becomes unreadable to every consuming skill | Service names are conventional — `open-agent-skills:<name>`. Don't fight it. |
